...
trait Grid[Element: ClassTag](val width: Int, val height: Int):
  ...
  private def isBetween(value: Int, low: Int, high: Int): Boolean = value >= low && value < high // From o1.util.nice

  /** Checks whether the grid contains the given x and y coordinates. */
  private def contains(x: Int, y: Int) = isBetween(x, 0, this.width) && isBetween(y, 0, this.height)
